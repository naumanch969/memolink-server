...
export async function runWeeklyAnalysis(userId: string): Promise<WeeklyAnalysisOutput> {
    ...
    // 1. Fetch Daily Reflections from the last 7 days
    ...
    // 3. Fetch Entries summary stats
    const entries = await Entry.find({
        userId,
        createdAt: { $gte: weekAgo }
    }).select('mood createdAt tags type').sort({ createdAt: 1 });

    // 3.5 Guard: If no data at all, return default content
    if (dailyReflections.length === 0 && entries.length === 0) {
        return {
    ...
    // 4. Prepare Context
    const reflectionsContext = dailyReflections.map(t => {
        const d = t.outputData as any;
        return `[${t.createdAt.toISOString().split('T')[0]}] Mood: ${d.moodScore}/5 - Summary: ${d.summary} - Themes: ${d.keyThemes?.join(', ')}`;
    }).join('\n');
