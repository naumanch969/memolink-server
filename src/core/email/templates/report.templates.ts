...
function baseLayout(title: string, preheader: string, body: string, frontendUrl: string): string {
    return `<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8"/>
...
export function getWeeklyReportEmailTemplate(content: any, period: string, frontendUrl: string): { subject: string; html: string } {
    ...
    const subject = score !== null ? `📊 ${headline} — Score: ${score}/100` : `📊 ${headline}`;
    ...
    return {
        subject,
        html: baseLayout(headline, `${headline} · Score ${score ?? ''}`, body, frontendUrl)
    };
}

export function getMonthlyReportEmailTemplate(content: any, period: string, frontendUrl: string): { subject: string; html: string } {
    const monthTitle = content?.monthTitle ?? "Your Monthly Reckoning is Ready";
    ...
    const subject = score !== null ? `📅 ${monthTitle} — Score: ${score}/100` : `📅 ${monthTitle}`;
    ...
    return {
        subject,
        html: baseLayout(monthTitle, `${monthTitle} · Your ${period} Reckoning`, body, frontendUrl)
    };
}
